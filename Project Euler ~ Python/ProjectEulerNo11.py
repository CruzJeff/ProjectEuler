# -*- coding: utf-8 -*-
"""
Created on Thu Sep 14 22:24:53 2017
...
for i in range(20):
    for j in range(16):
        result = M[i][j]*M[i][j+1]*M[i][j+2]*M[i][j+3]
        if result > MAX: 
            MAX = result
      
        result = M[j][i]*M[j+1][i]*M[j+2][i]*M[j+3][i]
        if result > MAX: 
            MAX = result
 
for i in range(16):
    for j in range(16):
        result= M[i][j]*M[i+1][j+1]*M[i+2][j+2]*M[i+3][j+3]
        if result > MAX: 
            MAX = result
